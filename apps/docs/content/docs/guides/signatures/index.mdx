...
## Sign with Cloud KMS / HSM

For enterprise environments requiring HSM-backed keys, use the cloud KMS signer for your provider.
...
const signer = await AzureKeyVaultSigner.create({
  ...
  certificate: certificateDer, // DER-encoded certificate for this key
  buildChain: true, // Auto-fetch intermediate certificates
});
...
See the [Azure Key Vault guide](/docs/guides/signatures/azure-key-vault) for complete setup instructions.
