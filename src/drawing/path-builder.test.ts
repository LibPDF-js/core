...
describe("PathBuilder", () => {
  ...
  describe("fillWithShading", () => {
    function createBuilderWithShading() {
      const appendContent = vi.fn();
      const registerGraphicsState = vi.fn(() => null);
      const registerShading = vi.fn(() => "Sh0");
      const registerPattern = vi.fn(() => "P0");
      ...
      return { builder, appendContent, registerShading, registerPattern };
    }

    it("fills path with shading using clipping", () => {
      const { builder, appendContent, registerShading } = createBuilderWithShading();

      const mockShading = {
        type: "shading" as const,
        ref: {} as any,
        shadingType: "axial" as const,
      };

      builder.rectangle(50, 50, 100, 100).fillWithShading(mockShading);

      expect(registerShading).toHaveBeenCalledWith(mockShading);
      expect(appendContent).toHaveBeenCalled();

      const content = appendContent.mock.calls[0][0];
      expect(content).toContain("q"); // Push graphics state
      expect(content).toContain("W"); // Clip
      expect(content).toContain("n"); // End path
      expect(content).toContain("/Sh0 sh"); // Paint shading
      expect(content).toContain("Q"); // Pop graphics state
    });

    it("throws if shading support not provided", () => {
      const { builder } = createBuilder();
      const mockShading = {
        type: "shading" as const,
        ref: {} as any,
      ...
      expect(() => builder.rectangle(50, 50, 100, 100).fillWithShading(mockShading)).toThrow(
        /fillWithShading requires shading support/,
      );
    });
  });

  describe("fillWithPattern", () => {
    function createBuilderWithPattern() {
      const appendContent = vi.fn();
      const registerGraphicsState = vi.fn(() => null);
      const registerShading = vi.fn(() => "Sh0");
      const registerPattern = vi.fn(() => "P0");
    ...
    it("fills path with pattern using pattern color space", () => {
      const { builder, appendContent, registerPattern } = createBuilderWithPattern();

      const mockPattern = {
        type: "pattern" as const,
        ref: {} as any,
        patternType: "tiling" as const,
      };

      builder.circle(100, 100, 50).fillWithPattern(mockPattern);

      expect(registerPattern).toHaveBeenCalledWith(mockPattern);
      expect(appendContent).toHaveBeenCalled();

      const content = appendContent.mock.calls[0][0];
      expect(content).toContain("q"); // Push graphics state
      expect(content).toContain("/Pattern cs"); // Set pattern color space
      expect(content).toContain("/P0 scn"); // Set pattern
      expect(content).toContain("f"); // Fill
      expect(content).toContain("Q"); // Pop graphics state
    });

    it("throws if pattern support not provided", () => {
      const { builder } = createBuilder();
      const mockPattern = {
        type: "pattern" as const,
        ref: {} as any,
      ...
      expect(() => builder.circle(100, 100, 50).fillWithPattern(mockPattern)).toThrow(
        /fillWithPattern requires pattern support/,
      );
    });
  });
});
